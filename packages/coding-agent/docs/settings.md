...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://pi.dev/api/report-install` (disabled in this fork). Opting out of telemetry does not disable update checks; Pi can still fetch `https://pi.dev/api/latest-version` (disabled in this fork) to look for the latest version.
