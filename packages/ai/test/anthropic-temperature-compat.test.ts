...
describe("Anthropic temperature compatibility", () => {
	it("omits temperature for Claude Opus 4.7", async () => {
		const payload = await capturePayload(getModel("anthropic", "claude-opus-4-7")! as Model<"anthropic-messages">, {
			temperature: 0,
		});
	...
	it("omits temperature for Claude Opus 4.8", async () => {
		const payload = await capturePayload(getModel("anthropic", "claude-opus-4-8")! as Model<"anthropic-messages">, {
			temperature: 0,
		});
	...
	it("keeps temperature for Claude Opus 4.6", async () => {
		const payload = await capturePayload(getModel("anthropic", "claude-opus-4-6")! as Model<"anthropic-messages">, {
			temperature: 0,
		});
	...
	it("keeps temperature for Claude Sonnet 4.6", async () => {
		const payload = await capturePayload(getModel("anthropic", "claude-sonnet-4-6")! as Model<"anthropic-messages">, {
			temperature: 0,
		});
